...
class ReferenceData extends EventEmitter {
  ...
  listFTPReferenceFiles() {
    return promisify(this.ftpClient.list)('/')
    ...
    //         .map(f => Object.assign(f, { path: `/${f.name}`}))
    //       );
    //   });
    // });
  }
  ...
  getFTPReferenceSize(fileInfo) {
    return new Promise((resolve, reject) => {
      this.ftpClient.size(fileInfo.path, (err, size) => {
        (err) ? reject(err) : resolve(Object.assign(fileInfo, { size }));
      })
    })
  }
      ...
      if (this.ftpClient.listenerCount('ready') === 0) {
        this.ftpClient
          ...
          .on('ready', (() => {
            ...
            this.updateInterval = setInterval(this.updateLocalReferenceData.bind(this), this.updateDelay);
...
//   gz.on('end', () => {
//     finalObj = parser.toJson(buffer.join(''), { object: true, coerce: true, reversible: false });
