      ...
      return new Promise((resolve, reject) => {
        ...
        gz.on('end', () => {
          ...
          promisify(this, parseString, buffer.join(''), parserOptions)
            .then((result) => {
              console.log(downloadInfo.filePath);
              return fs.writeJson(downloadInfo.filePath, result)
            })
            .then(() => {
              const finalFileInfo = {
                name: path.basename(fileInfo.name, '.gz'),
                path: path.join(localReferenceDataDir, path.basename(fileInfo.name, '.gz'))
              };
              this.emit('referenceDataUpdate', finalFileInfo);
