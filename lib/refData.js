...
_getRemoteStream(filePath, stream, size, resolve, reject) {
          ...
          this._ftpClient.size(v3File.name, (err, size) => {
            if (err) {
              ...
              this._ftpClient.get(v3File.name, (err, stream) => {
                (err) ? reject(err) : this._getRemoteStream(this.v3Loc, stream, size, resolve, reject);
              });
            }
          });
        } else {
          ...
          this._ftpClient.size(v8File.name, (err, size) => {
            if (err) {
              ...
              this._ftpClient.get(v8File.name, (err, stream) => {
                (err) ? reject(err) : this._getRemoteStream(this.v8Loc, stream, size, resolve, reject);
              });
            }
          });
        } else {
  ...
  console.log(Object.getOwnPropertyNames(module.exports));
    return new Promise((resolve, reject) => {
      getLocalTimetableIds().then((manifest) => {
