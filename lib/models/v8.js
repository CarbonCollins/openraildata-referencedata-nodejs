...
const { Association, Schedule } = require('openraildata-common');
...
class V8RefData {
  ...
  parseV8(payload) {
    if (payload && payload.PportTimetable) {
      ...
      this._jurneys = payload.PportTimetable.Journey.map(journey => new Schedule(journey));
      this._associations = payload.PportTimetable.Association.map(assoc => new Association(assoc));
    }
  }
