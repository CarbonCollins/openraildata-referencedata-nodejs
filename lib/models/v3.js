...
class V3RefData {
  ...
  constructor(refData = {}) {
    ...
    this.locations = this.payload.LocationRef;
    this.trainOperatorCompanies = this.payload.TocRef;
   ...
   */
  get timetableId() {
    return (this.payload.$ && this.payload.$.timetableId)
  ...
  set locations(locationArr) {
    this._locations = (Array.isArray(locationArr))
      ? locationArr
        .map(loc => new Location(loc.$ || loc || {}))
      : [];
  }
  ...
  get trainOperatorCompanies() {
    return this._trainOperatorCompanies;
  }
  set trainOperatorCompanies(trainOperatorCompaniesArr) {
    this._trainOperatorCompanies = (Array.isArray(trainOperatorCompaniesArr))
      ? trainOperatorCompaniesArr
        .map(toc => new TOC(toc.$ || toc || {}))
      : [];
  }
